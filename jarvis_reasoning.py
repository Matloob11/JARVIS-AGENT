...
class IntentAnalyzer:
    ...
    def analyze_intent(self, text: str) -> Dict[str, Any]:
        ...
        primary_intent = "general" if not detected_intents else max(confidence_scores, key=confidence_scores.get)
...
async def analyze_user_intent(user_input: str) -> Dict[str, Any]:
    ...
    """
    try:
        logger.info(f"Analyzing intent for: {user_input[:50]}...")
        
        # Perform intent analysis
        intent_result = intent_analyzer.analyze_intent(user_input)
        
        logger.info(f"Primary intent detected: {intent_result['primary_intent']}")
...
async def generate_smart_response(user_input: str, intent_analysis: Dict, memory_context: List) -> str:
    """
    ...
    try:
        ...
        # Analyze context
        context_info = context_analyzer.analyze_context(user_input, memory_context)
        
        # Generate response
        primary_intent = intent_analysis.get("primary_intent", "general")
        response = response_generator.generate_response(primary_intent, context_info, user_input)
        ...
        logger.info(f"Generated response with reasoning: {reasoning_info}")
        
        return response
        
    except Exception as e:
        logger.error(f"Error in response generation: {e}")
        return "Sir Matloob, main aapki baat samajh gaya hun. Kaise madad kar sakta hun?"

async def process_with_advanced_reasoning(user_input: str, conversation_history: List = None) -> Dict[str, Any]:
    """
    Complete reasoning pipeline
    """
    try:
        # Step 1: Intent Analysis
        intent_result = await analyze_user_intent(user_input)
