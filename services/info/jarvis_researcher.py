...
async def perform_web_research(query: str) -> str:
    ...
    if not urls:
        msg = "Maaf Sir, main research ke liye links nahi dhoond paya. Search API check karein."
        return msg
...
async def autonomous_research_and_email(query: str, recipient: str,
    ...
    email_body = f"{intro}{research_summary}"

    email_result = await send_email(recipient, subject, email_body)

    if email_result.get("status") == "success":
        result_msg = (
            f"✅ Agentic task complete: Research karke report '{recipient}' "
            f"ko bhej di gayi hai."
        )
        return {
            "status": "success",
            "query": query,
    ...
    fail_msg = (
        f"⚠️ Research to ho gayi, par email bhejne mein masla aaya: "
        f"{email_result.get('message')}"
    )
    return {
